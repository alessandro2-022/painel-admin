...
import { getPromotions, createPromotion, updatePromotion, togglePromotionStatus } from '../services/apiService.ts';
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
                <h2 className="text-2xl font-semibold text-slate-900 mb-6">Gerenciar Promoções</h2>
                
                {/* Filtro de Data */}
                <div className="mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                        <div className="lg:col-span-2">
                            <label htmlFor="startDate" className="block text-sm font-medium text-slate-600">Criadas a partir de</label>
                            <input type="date" name="startDate" id="startDate" value={dateFilter.startDate} onChange={handleFilterChange} className="mt-1 block w-full p-2 border-slate-300 rounded-md shadow-sm focus:ring-[#0057b8] focus:border-[#0057b8] text-slate-900"/>
                        </div>
                        <div className="lg:col-span-2">
                            <label htmlFor="endDate" className="block text-sm font-medium text-slate-600">Até</label>
                            <input type="date" name="endDate" id="endDate" value={dateFilter.endDate} onChange={handleFilterChange} className="mt-1 block w-full p-2 border-slate-300 rounded-md shadow-sm focus:ring-[#0057b8] focus:border-[#0057b8] text-slate-900"/>
                        </div>
                        <div className="flex gap-2 lg:col-span-1">
                            ...
                            <button onClick={handleClearFilter} className="w-full px-4 py-2 bg-slate-200 text-slate-700 font-semibold rounded-lg hover:bg-slate-300 transition-colors">Limpar</button>
                        </div>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-200">
                                <th className="p-3 text-sm font-semibold text-slate-700">Código</th>
                                <th className="p-3 text-sm font-semibold text-slate-700">Desconto</th>
                                <th className="p-3 text-sm font-semibold text-slate-700">Público</th>
                                <th className="p-3 text-sm font-semibold text-slate-700">Status</th>
                                <th className="p-3 text-sm font-semibold text-slate-700 text-right">Ações</th>
                            </tr>
                        </thead>
                        <tbody>
                            {isLoading ? (
                                <tr>
                                    <td colSpan={5} className="text-center p-4 text-slate-500">Carregando promoções...</td>
                                </tr>
                            ) : error ? (
                                <tr>
                                ...
                                paginatedPromotions.map(promo => (
                                    <tr key={promo.id} className="border-b border-slate-200 hover:bg-slate-50">
                                        <td className="p-3 font-mono text-slate-900">{promo.code}</td>
                                        <td className="p-3 font-medium text-slate-900">{promo.discount}%</td>
                                        <td className="p-3 capitalize text-slate-700">{promo.target === 'user' ? 'Usuários' : 'Motoristas'}</td>
                                        <td className="p-3">
                                            <label htmlFor={`toggle-${promo.id}`} className="flex items-center cursor-pointer">
                                                <div className="relative">
                                                    ...
                                                    <div className={`block w-14 h-8 rounded-full transition ${promo.isActive ? 'bg-[#0057b8]' : 'bg-slate-300'}`}></div>
                                        ...
                                        <td className="p-3 text-right space-x-2">
                                            <button onClick={() => setHistoryModalPromo(promo)} className="p-2 text-slate-500 hover:text-slate-700 rounded-full hover:bg-slate-200 transition-colors" aria-label="Ver histórico">
                                                <HistoryIcon className="h-5 w-5"/>
                                            </button>
                                            <button onClick={() => setEditingPromo(promo)} className="p-2 text-slate-500 hover:text-[#0057b8] rounded-full hover:bg-slate-200 transition-colors" aria-label="Editar">
                                                <EditIcon />
                                            </button>
                                        </td>
                                    </tr>
                                ...
                                <tr>
                                    <td colSpan={5} className="text-center p-4 text-slate-500">Nenhuma promoção encontrada para os filtros selecionados.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
            ...
            <div className="bg-white p-8 rounded-xl shadow-md border border-slate-200">
                ...
                <form ref={formRef} onSubmit={handleFormSubmit} className="space-y-6">
                    <div>
                        <label htmlFor="code" className="block text-sm font-medium text-slate-600">Código Promocional</label>
                        <input
                            type="text"
                            id="code"
                            name="code"
                    ...
                    <div>
                        ...
                        <div className="mt-2 flex space-x-4">
                            <label className="flex items-center cursor-pointer">
                                ...
                                <span className="ml-2 text-slate-700">Usuários</span>
                            </label>
                             <label className="flex items-center cursor-pointer">
                                ...
                                <span className="ml-2 text-slate-700">Motoristas</span>
                            </label>
                        </div>
                    </div>
                     <div className="pt-4 flex justify-end items-center gap-4">
                         {showSuccess && <p className="text-green-600 transition-opacity duration-300">{showSuccess}</p>}
                         {editingPromo && (
                             <button type="button" onClick={handleCancelEdit} className="px-6 py-3 bg-slate-200 text-slate-800 font-semibold rounded-lg hover:bg-slate-300 transition-colors">
                                 Cancelar
                             </button>
                         )}
                        <button
            ...
            {historyModalPromo && (
                ...
                >
                    <div 
                      className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-scale-up" 
                      onClick={e => e.stopPropagation()}
                    >
                        <header className="p-6 border-b border-slate-200">
                            <h2 id="promo-history-title" className="text-2xl font-bold text-slate-900">
                        ...
                        <main className="p-6 flex-1 overflow-y-auto">
                            <div className="text-sm space-y-2 mb-6 bg-slate-50 p-4 rounded-lg">
                                <p><span className="font-semibold text-slate-700">Criada em:</span> {formatDate(historyModalPromo.createdAt)}</p>
                                <p><span className="font-semibold text-slate-700">Última Atualização:</span> {formatDate(historyModalPromo.updatedAt)}</p>
                            </div>

                            <ul className="border-l-2 border-slate-200 ml-2 space-y-6">
                                {historyModalPromo.history.map((entry, index) => (
                                    <li key={index} className="relative pl-8">
                                        <div className="absolute -left-[11px] top-1 w-5 h-5 bg-[#0057b8] rounded-full border-4 border-white flex items-center justify-center">
                                          <HistoryIcon className="w-3 h-3 text-white" />
                                        </div>
                                        <p className="text-sm font-semibold text-slate-900">{formatDate(entry.date)}</p>
                                        <p className="text-slate-700">{entry.change}</p>
                                    </li>
                                ))}
                            </ul>
                        </main>
                        <footer className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end">
                            <button onClick={() => setHistoryModalPromo(null)} className="px-5 py-2 bg-slate-200 text-slate-800 font-semibold rounded-lg hover:bg-slate-300 transition-colors">
                                Fechar
                            </button>
                        </footer>
                    </div>
