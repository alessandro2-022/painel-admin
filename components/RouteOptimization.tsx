...
import { Stop, OptimizedRoute } from '../types';
...
const RouteOptimization: React.FC = () => {
    ...
    useEffect(() => {
        ...
        stops.forEach((stop, index) => {
            ...
            label.className = `w-6 h-6 rounded-full bg-slate-800 text-white flex items-center justify-center text-xs font-bold border-2 border-white shadow-md`;
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full">
            <div className="lg:col-span-1 bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md flex flex-col dark:border dark:border-slate-700">
                <h2 className="text-xl font-semibold mb-4 text-slate-800 dark:text-slate-100">Planeje sua Rota</h2>
                
                <input
                    ref={addressInputRef}
                    type="text"
                    className="w-full p-3 border-slate-300 dark:border-slate-600 rounded-md focus:ring-[#0057b8] focus:border-[#0057b8] dark:bg-slate-700 dark:text-slate-200 dark:placeholder-slate-400"
                ...
                <div className="flex-1 mt-4 overflow-y-auto -mr-3 pr-3">
                    <ul className="space-y-2">
                        {stops.map((stop, index) => (
                            <li key={stop.id} className="flex items-center justify-between p-3 bg-slate-100 dark:bg-slate-900/50 rounded-md">
                                <div className="flex items-start">
                                    <span className="text-sm font-bold text-slate-500 dark:text-slate-400 w-6 pt-px">{index + 1}.</span>
                                    <span className="text-slate-700 dark:text-slate-300 flex-1">{stop.address}</span>
                                </div>
                                <button onClick={() => handleRemoveStop(stop.id)} className="text-slate-400 hover:text-red-500 dark:hover:text-red-400 ml-2 flex-shrink-0">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                </button>
                            </li>
                        ))}
                         {stops.length === 0 && (
                            <li className="text-center p-4 text-slate-500 dark:text-slate-400">Nenhuma parada adicionada.</li>
                        )}
                    </ul>
                </div>
                ...
                <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-700 space-y-2">
                     <button
                        ...
                        className="w-full px-6 py-3 bg-[#0057b8] text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed transition-colors"
                    >
                    ...
                    <button
                        ...
                        className="w-full px-6 py-2 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 font-semibold rounded-lg hover:bg-slate-300 dark:hover:bg-slate-500 disabled:bg-slate-400/50 dark:disabled:bg-slate-600/50 disabled:cursor-not-allowed transition-colors"
                    >
                        Limpar Tudo
                    </button>
                </div>
            ...
            <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md flex flex-col dark:border dark:border-slate-700">
                <div className="flex-1 flex flex-col">
                    {optimizedRoute ? (
                        <div className="mb-4">
                            <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-100">Rota Otimizada</h2>
                            <div className="flex space-x-8 text-center mt-2 p-4 bg-slate-100 dark:bg-slate-900/50 rounded-lg">
                                <div className="flex-1">
                                    <p className="text-sm text-slate-500 dark:text-slate-400">Distância Total</p>
                            ...
                            <ol className="list-decimal list-inside space-y-1 text-slate-700 dark:text-slate-300 mt-4 text-sm">
                                {optimizedRoute.stops.map(stop => <li key={stop.id}>{stop.address}</li>)}
                            </ol>
                        </div>
                    ) : (
                        <div className="text-center mb-4">
                            <h2 className="text-xl font-semibold text-slate-600 dark:text-slate-300">Visualização da Rota</h2>
                            <p className="text-slate-400 dark:text-slate-500 mt-1">O mapa será atualizado à medida que você adiciona paradas.</p>
                        </div>
                    )}
                    
                    <div className="flex-1 w-full bg-slate-200 dark:bg-slate-900/50 rounded-lg relative overflow-hidden min-h-[300px]">
                        ...
                        {isLoading && (
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm z-10">
                                ...
                                <p className="mt-2 text-slate-600 dark:text-slate-300 font-semibold">Otimizando Rota...</p>
                            </div>
                        )}
                    </div>
                </div>
